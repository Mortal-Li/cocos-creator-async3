 ...
 * Socket异步封装
 * @author Mortal-Li
 * @created Sat Apr 27 2024 21:09:17 GMT+0800 (中国标准时间)
 */
    ...
    connectAsync(options: IConnectOptions, connectedCB?: Function) {
        if (this._connectOptions == null) {
            this._connectOptions = {
                ...
                manualReconnect: options.manualReconnect ? options.manualReconnect : () => warn("manualReconnect undefined")
            };
        ...
        return new Promise<void>((resolve, reject) => {
            if (this._ws) {
                if (this._ws.readyState === WebSocket.CONNECTING) {
                    log("ws connecting, wait for a moment...");
                    resolve();
                    return ;
                } else if (this._ws.readyState === WebSocket.OPEN) {
                    log("ws has connected!");
                    resolve();
                    return ;
                }
            }

            log("net is connecting!");
